...
import org.code.blocks.common.protocol.base.TreeNode;
...
public class TreeUtil {
    ...
    public static Map<String, List<Integer>> getMap(List<Menu> list) {
        ...
        if(list == null || list.isEmpty()) {
            throw new BaseException("参数不能为null");
        }
    ...
    private static List<TreeNode> buildTreeByColumn(List<Menu> list, String nodeId, Boolean isParent) {
        ...
        if (list == null || list.isEmpty()) {
            throw new BaseException("资源列表不能为null");
        }
        if (StringUtils.isBlank(nodeId)) {
            throw new BaseException("nodeId不能为null");
        }
